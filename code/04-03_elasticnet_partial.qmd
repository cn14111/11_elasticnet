...
Ridge doesn't set to zero but closer to zero. Lasso set to zero (lasso tells the model which variables must set to zero.)
...
Let's check the distribution of our predicted variable **strength_gtex** across training and testing: to avoid data shift 
```{r distribution}
ggplot() +
...
What penalty and mixture values created lowest RMSE?  penalty of 1 and mixture of 0
...
```
How does metrics on test compare to metrics on train?  rmse is smaller in training set than test set and rsq is higher in training set and smaller in test set
